...
| [ADR-0010](0010-cross-platform-reproducibility.md) | 跨平台一致性：conda-lock 双平台锁文件 + 单点 OS 检测 | accepted | 项目同时跑在 Mac（osx-arm64）与 Linux 服务器（linux-64）；用 conda-lock 为双平台生成统一锁文件保证版本一致，无法对齐的极少数包登记 `docs/cross-platform-exceptions.md`，所有 OS 检测收口到 `src/scrna_integration/platform.py` 单一模块 |
