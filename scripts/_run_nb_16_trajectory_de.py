#!/usr/bin/env python
"""Execute 07_downstream/16_trajectory_de.ipynb cell by cell."""
...
nbPath = 'notebooks/07_downstream/16_trajectory_de.ipynb'
# Load notebook
nb = nbformat.read(nbPath, as_version=4)
code_cells = [(i, c) for i, c in enumerate(nb.cells) if c.cell_type == 'code']

print(f'=== {nbPath} ({len(code_cells)} code cells) ===')
...
print(f'\n=== {nbPath} SUMMARY: {passed} PASS, {failed} FAIL, {skipped} SKIP ===')
for r in results:
    status = '✓' if r[2] == 'PASS' else ('✗' if r[2] == 'FAIL' else '-')
