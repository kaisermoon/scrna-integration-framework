...
- **src 与 notebook 边界铁律**（2026-07-10 PI 定稿，见 `docs/优化方案-20260710.md` 第一节）：**进 `src/scrna_integration/` 的只有"无科研价值的技术管道"，一切分析逻辑留 notebook cell，可见、可调、可开关。** 技术管道限四类：① LLM 配置与调用（`llm_config.py`）② 平台检测收口（`platform.py`：OS / CUDA / Rscript 路径）③ 启动脚手架（找项目根 + BLAS 线程设置）④ 跨语言桥接（rds 经 R 读入这一步）。分析逻辑（数据读取与字段映射、QC、标准化、细胞周期打分、评分指标计算、marker 加载筛选、所有绘图）一律留 notebook cell，重复出现不视为缺陷。此铁律比"薄框架"更进一步：薄框架讲"少写 wrapper"，本条限定"wrapper 只允许上述四类技术管道，其余一律 notebook"。可选步骤整段包在 `if XXX_ENABLED:` cell 里，用则打开、不用则跳过或删除。`platform.py` 只放检测函数，notebook 调用后必须 `print` 出决策结果（选了什么设备、CUDA 是否可用、Rscript 路径），让决策过程在 cell 里显示。reviewer 卡这条。
...
- **状态文件公开仓纪律**：本仓为 public，`_memory.md`/`_plan.md` 已纳入 git 跟踪并同步至公开仓。此纪律同时约束**文件正文、commit message、PR 标题与正文**（三处均公开可见，2026-07-10 事故正是栽在 commit message + PR 描述带真实人名）。禁止出现：① 真实绝对路径（用 `~`/项目内相对路径/`<vault根>` 占位）② 内网地址/端口/网关 IP/任何凭证（用 `<本地网关>` 占位，密钥永不入库）③ 对协作方的指责性措辞（违纪/谎报等 → 改写为中性技术教训，教训保留）④ **真实人名（研究生/合作方/领导）与未发表课题信息（内部课题名、合作关系、方药组合、疾病方向绑定等）**——这类内容属项目组内部隐私，写入前一律用角色代称（如"某研究生"）或直接省略，绝不入公开仓任何位置。人名/未发表信息一旦 push 即不可逆（不像密钥可轮换），写入前必须停下自检。
